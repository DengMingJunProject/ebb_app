...
//		err = 1;
    ...
    if( err )
        DBG_LOG( "LittleFS mounte Fail(%d)!!\r\n", err );
    else
        DBG_LOG( "LittleFS mounted\r\n" );
    //ls();
}
...
int midfs_isexist(char *name )
{
	int size = 0,res = 0;
	FILE_HANDLER fd;
	res = midfs_fopen(&fd,name,"r");
	if(res >= 0 )
	{
		size = midfs_fsize(&fd);
		midfs_fclose(&fd);
...
//        midfs_check();
        ...
        midfs_ls();
        DBG_LOG( "MID FS init done!\r\n" );
		sys_status.have_spi_flash = 1;
    }
}
